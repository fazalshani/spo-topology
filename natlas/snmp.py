...
class natlas_snmp:
    ...
    def cache_lookup(self, varBindTable, name):
        print('natlas_snmp ==> cache_lookup()')
    ...
    def get_last_oid_token(oid):
        print('natlas_snmp ==> get_last_oid_token()')
        _oid = oid.getOid()
        ts   = len(_oid)
        return _oid[ts-1]
