...
class util:
    ...
    def get_net_bits_from_mask(netm):
        print('util ==> get_net_bits_from_mask()')
        cidr = 0
        mt = netm.split('.')
        for b in range(0, 4):
            v = int(mt[b])
    ...
    def get_module_from_interf(port):
        print('util ==> get_module_from_interf()')
        try:
    ...
    def strip_slash_masklen(cidr):
        print('util ==> strip_slash_masklen()')
        try:
    ...
    def expand_path_pattern(str):
        print('util ==> expand_path_pattern()')
        try:
            match = re.search('{([^\}]*)}', str)
